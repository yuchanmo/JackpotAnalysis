...
exist_df = pd.read_sql('select * from dbo.Code',sqlserver)
joincols = list(code_df)
joincols.remove('issuedate')
n,e = filterNewExistTable(code_df,exist_df,joincols,joincols,'Id',False)
n.to_sql('Code',sqlserver,'dbo',if_exists='append',index=False)
