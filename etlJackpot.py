...
min_max_df = df.groupby(['code']).apply(recent2Maxim2Minimum).reset_index()
min_max_df.columns = ['code','minmax']

final_df= pd.merge(issuedf,min_max_df,left_on=['code'],right_on=['code'],how='left')
final_df_cols = ['Regdate','cate', 'code', 'name', 'startval', 'startvol','curval', 'curvol','startvol_curvol_diff', 'startval_curval_diff',  'avg3', 'avg7', 'val_infle', 'vol_infle', 'curval_1diff', 'curvol_1diff', 'avg3_1diff', 'avg7_1diff', 'curval_3diff', 'curvol_3diff', 'avg3_3diff', 'avg7_3diff', 'minmax']
